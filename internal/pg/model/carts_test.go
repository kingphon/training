...
func testCartToManyCartItems(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := CartSlice{&a}
	if err = a.L.LoadCartItems(ctx, tx, false, (*[]*Cart)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCartToManyAddOpCartItems(t *testing.T) {
	var err error
...
var (
	cartDBTypes = map[string]string{`ID`: `text`, `UserID`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
