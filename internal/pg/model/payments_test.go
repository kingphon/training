...
func testPaymentToManyOrders(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadOrders(ctx, tx, false, (*[]*Payment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPaymentToManyAddOpOrders(t *testing.T) {
	var err error
...
func testPaymentToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Payment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testPaymentToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	paymentDBTypes = map[string]string{`ID`: `text`, `Code`: `text`, `Method`: `text`, `Amount`: `numeric`, `Status`: `text`, `UserID`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_              = bytes.MinRead
)
